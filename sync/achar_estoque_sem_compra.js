...
async function achar(sheets) {
  ...
  const cdt = header.indexOf('DATA DA COMPRA');

  const client = await pool.connect();
  console.log('Linha | IMEI | Produto | Fornecedor | Custo | Data | Motivo');
  let total = 0;
  let somaFaltante = 0;
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (motivo) {
      total++;
      somaFaltante += custo;
      const custo_total = parseMoeda(rows[i][header.indexOf('CUSTO TOTAL')]);
      console.log(`Linha ${i+6} | ${imei||'---'} | ${rows[i][cp]} | ${rows[i][cf]} | R$ ${custo_total||custo} | ${rows[i][cdt]} | ${motivo}`);
    }
  }
  console.log('---');
  console.log('Total: ' + total + ' | Soma custo: R$ ' + somaFaltante.toFixed(2));
  client.release();
  await pool.end();
}
